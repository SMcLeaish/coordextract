...
@pytest.mark.asyncio
@pytest.mark.parametrize("gpx_file_path, concurrency, raises_error,\
expected_result", [
...
async def test_process_gpx(
    ...
    with patch.object(GPXUtils, 'parse_gpx',\
                      new=MagicMock(return_value=expected_result))\
        as mock_parse_gpx, \
        ...
        mock_aiofiles_open.return_value = AsyncMock(__aenter__=AsyncMock\
...
@pytest.mark.parametrize("test_condition, raises_error,\
...
def test_parse_gpx(test_condition: str, raises_error: bool,\
...
@pytest.mark.parametrize("points_with_types, concurrent, expected_call_count",\
[
    ({"wpt": [(10.0, -20.0, {"name": "Test Waypoint"})]}, False, 1),
])
def test_build_gpx_point_models(points_with_types: Parsedgpx, concurrent:bool,\
                                expected_call_count: int, mock_point_model:\
                                MagicMock)-> None:
    """Test the _build_gpx_point_models method of the GPXUtils class."""
    with patch('coordextract.point.PointModel.build', return_value=\
               mock_point_model()) as mock_build:
        #pylint: disable=protected-access
        result = GPXUtils._build_gpx_point_models(points_with_types,\
                                                  concurrent)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            (40.6892, -74.0445, {"name": "Liberty Island"}), False,
        ),
...
def test_parse_point(
    xml_input: str, expected_output: tuple[float|str, float, dict[str, str]] |\
    None, raises_error: bool
    ) -> None:
