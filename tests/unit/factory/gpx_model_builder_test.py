...
@pytest.mark.asyncio
@pytest.mark.parametrize("return_value, expected_length", [
    (([(25.0, 3.0)], [], []), 1),
...
async def test_process_gpx_to_point_models_with_data  \
    (mock_latlon_to_mgrs, mock_async_parse_gpx, \
    ...
    assert len(result) == expected_length, f"Expected {expected_length} PointModel(s) to be created"
