...
@pytest.mark.parametrize(
    ...
    [
        ...
        (37.65815587109628, -101.45319156731206, "00AAB1234567890",\
         "invalid_mgrs"),
...
def test_conversion_functions(
    ...
    if test_condition == "valid":
        with patch('coordextract.conversion_utils.validate_mgrs',\
                   return_value=True):
        ...
        with patch('coordextract.conversion_utils.validate_mgrs',\
                   return_value=False), pytest.raises(MGRSError) as excinfo:
            lat_lon_to_mgrs(latitude, longitude)
        assert "Invalid MGRS string returned by mgrs.toMGRS" in\
        ...
        if test_condition == "bad_latitude":
            ...
            with patch ('coordextract.conversion_utils.validate_latitude',\
                        return_value=False), pytest.raises(MGRSError) as\
                excinfo:
                mgrs_to_lat_lon(mgrs)
            assert "Invalid latitude" in str(excinfo.value.__cause__)
        elif test_condition == "bad_longitude":
            assert "Invalid longitude" in str(excinfo.value.__cause__)
            with patch ('coordextract.conversion_utils.validate_longitude',\
                        return_value=False), pytest.raises(MGRSError) as\
                excinfo:
                mgrs_to_lat_lon(mgrs)
            assert "Invalid longitude" in str(excinfo.value.__cause__)
